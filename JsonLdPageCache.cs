...
namespace NuGet3.Client.Core
{
    ...
    {
        ConcurrentDictionary<Uri, object> _pages = new ConcurrentDictionary<Uri, object>();

        CollectorHttpClient _client;

        public JsonLdPageCache(CollectorHttpClient client)
        {
            _client = client;
        }

        public async Task<JToken> Fetch(Uri address)
        {
            object maybePage = _pages.GetOrAdd(address, new Func<Uri,Task<Page>>(async (Uri newPageAddress) =>
            {
                string json = await _client.GetStringAsync(newPageAddress);
            ...
            {
                page = await (Task<Page>)maybePage;
                _pages[address] = page;
            }
